...
class CanaryPredictRequest(BaseModel):
    error_rate: float
    latency_p99: float
    saturation: float


class CanaryPredictResponse(BaseModel):
    decision: str
    explanation: str
...
@app.post("/canary-predict", response_model=CanaryPredictResponse, tags=["Prediction"])
def canary_predict(request: CanaryPredictRequest):
    """
    Accepts Canary metrics and returns:
    - Guardrail decision
    - AI explanation (if HF_TOKEN is set)
    """
    try:
        decision = check_guardrail(
            error_rate=request.error_rate,
            latency_p99=request.latency_p99,
            saturation=request.saturation
        )
        
        explanation = "No explanation available. Please set HF_TOKEN environment variable."
...
Canary metrics:
Error rate: {request.error_rate}%
Latency P99: {request.latency_p99} ms
Saturation: {request.saturation}%
...
"""
                ...
                explanation = f"Error getting explanation from AI: {e}"

        return CanaryPredictResponse(
            decision=decision,
